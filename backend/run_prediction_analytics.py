...
def bucket_summary(rows: list[dict]) -> list[dict]:
    ...
    for b in sorted(grouped.keys()):
        ...
        def _wr(tp_col: str, _grp: list[dict] = grp) -> float:
            wins = sum(1 for r in _grp if r.get(tp_col) == "tp")
            losses = sum(1 for r in _grp if r.get(tp_col) == "sl")
            denom = wins + losses
        ...
        out.append({
            ...
            "winrate_tp15_sl15": round(_wr("tp15_sl15_exit_reason"), 6),
            "winrate_tp15_sl25": round(_wr("tp15_sl25_exit_reason"), 6),
            "winrate_tp15_sl30": round(_wr("tp15_sl30_exit_reason"), 6),
...
def build_detailed_rows_for_modes(mode_rows: dict[str, list[dict]]) -> list[dict]:
    """Flatten mode rows into deterministic detailed rows with mode marker."""
    detailed_rows: list[dict] = []
    for mode, rows in mode_rows.items():
        for row in rows:
            row_with_mode = dict(row)
            row_with_mode["resolution_mode"] = mode
            detailed_rows.append(row_with_mode)
    detailed_rows.sort(
        key=lambda r: (r["timestamp"], r["event_id"], r["resolution_mode"])
    )
    return detailed_rows


def compute_entry_fallback_stats(pred_rows: list[dict]) -> tuple[int, float]:
    """Return fallback count and rate from detailed prediction rows."""
    fallback_count = sum(
        1 for row in pred_rows if row.get("entry_price_is_fallback")
    )
    fallback_rate = fallback_count / len(pred_rows) if pred_rows else 0.0
...
def get_detailed_fieldnames() -> list[str]:
    ...
    return [
        ...
        "entry_price_at_prediction", "entry_vs_level_offset", "entry_price_is_fallback", "entry_timestamp",
...
def run_prediction_analytics(
    ...
    def _on_prediction(prediction):
        ...
        entry_price_is_fallback = state["latest_price"] is None
        entry_price = (
            float(prediction.level_price)
            if entry_price_is_fallback
            else float(state["latest_price"])
        )
        ...
        row = PredictionRow(
            ...
            entry_vs_level_offset=entry_price - float(prediction.level_price),
    ...
    for pred in sorted_predictions:
        ...
        row = {
            ...
            "entry_vs_level_offset": round(pred.entry_vs_level_offset, 4),
    ...
    fallback_count, fallback_rate = compute_entry_fallback_stats(pred_rows)
    summary_rows.append(
        {
    ...
    detailed_fieldnames = get_detailed_fieldnames()

    # Write detailed rows for all requested resolution modes.
    detailed_rows = build_detailed_rows_for_modes(
        {mode: mode_rows[mode] for mode in mode_list}
    )
    ...
    print(f"Detailed CSV (all requested modes): {detailed_csv}")
    print(f"Summary CSV: {summary_csv}")
    print(
        "Entry price fallback usage: "
